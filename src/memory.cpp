...
#pragma optimize("", off)
		bool detour(BYTE* old_func, BYTE* new_func, const DWORD len)
		{
			...
			*reinterpret_cast<DWORD*>(new_mem4_base_end + 0x01) = DWORD((old_func + 10) - new_mem4_base_end - 5);
			...
			std::cout << std::hex << "Detour> filling remaining 0x" << (len - 10) << " real function bytes with NOPs" << std::
				dec << std::endl;
		...
		DWORD find_pattern(const BYTE* b_mask, const char* sz_mask)
		{
			...
			for (DWORD i = 0; i < dw_len; i++)
				if (data_compare(reinterpret_cast<unsigned char*>(dw_address + i), b_mask, sz_mask))
					return dw_address + i;
			return 0;
		}
